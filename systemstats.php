...
# Closed Libraries
$ClosedLibraryQuery = "SELECT * FROM `$cdlcLIB` where  participant = 0 OR `ill_email` = '' OR alias = '' ";
...
$NonPartLibrariesQuery = "SELECT * FROM `$cdlcLIB` where participant = 0 ";
$RetVal = mysqli_query($db, $NonPartLibrariesQuery);
$NonPartLibraries = mysqli_num_rows($RetVal);
# Suspended Libraries
$SuspendedLibrariesQuery = "SELECT * FROM `$cdlcLIB` where suspend = 1 and participant = 1 ";
$RetVal = mysqli_query($db, $SuspendedLibrariesQuery);
$SuspendedLibraries = mysqli_num_rows($RetVal);

# Configuration Problems
$ConfigProblemsQuery = "SELECT * FROM `$cdlcLIB` where `ill_email` = '' and participant = 1  ";
$RetVal = mysqli_query($db, $ConfigProblemsQuery);
$ConfigProblemsEmail = mysqli_num_rows($RetVal);

# Configuration Problems ALIAS missing
$ConfigProblemsQuery = "SELECT * FROM `$cdlcLIB` where alias = '' and participant = 1 ";
$RetVal = mysqli_query($db, $ConfigProblemsQuery);
$ConfigProblemsAlias = mysqli_num_rows($RetVal);

#Active Stats -> Requested materials
$RequestedMaterialsQuery = "SELECT distinct(`Requester LOC`) FROM `$cdlcSTAT` where Timestamp between date_sub(now(), interval 30 day) and now(); ";
...
$RespondedRequestsQuery = "SELECT distinct(Destination) FROM `$cdlcSTAT` where Timestamp between date_sub(now(), interval 30 day) and now() AND Fill < 2 ";
