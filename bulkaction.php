...
if (isset($_POST['bulkaction'])) {
    ...
    foreach ((array) $_POST['check_list'] as $id) {
        ...
        if ($action==1) {
            $sqlupdate = "UPDATE `$cdlcSTAT` SET `Fill` =  '6' WHERE `illNUB` = '$id'";
            ...
            $sqlupdate = "UPDATE `$cdlcSTAT`  SET `renewTimeStamp` = '".$timestamp."', `renewAccountRequester` = '" .$wholename."' WHERE `illNUB` = '$id'";
            if (mysqli_query($db, $sqlupdate)) {
            ...
            $sqlupdate = "UPDATE `$cdlcSTAT` SET `receiveTimeStamp` = '$timestamp', `receiveAccount` = '" .$wholename."', `receiveDate` = '$todaydate' WHERE `illNUB` = '$id'";
            if (mysqli_query($db, $sqlupdate)) {
