...
VALUES ('0','$ititle','$iauthor','$pubdate','$isbn','$issn','$itype','$itemcall','$itemlocation','$itemavail','$article','$needbydate','$reqnote','$destloc','$destsystem','$inst','$reqLOCcode','$reqsystem','$fname $lname','$email','$today','3','','$wphone','$saddress','$caddress','','','','','','')";
    ...
    if (mysqli_query($db, $sql)) {
        ...
        if ($libilliad=='1') {
            $sqlseloclc = "SELECT loc,name,`ILL Email`,address2,address3,OCLC,`system` FROM `$cdlcLIB` WHERE `loc`='$reqLOCcode'";
            //echo $sqlseloclc ;
            ...
            if ($reqLOCcode=='nnepsu') {
                ...
                if (empty($arttile)) {
                    ...
                    $jsonstr = array( 'Username' =>'Lending','WantedBy'=>'CDLC eForm Request: This is a book loan','LendingString'=> 'This is a book loan', 'ProcessType'=>Lending,'LenderAddressNumber'=>$illiadADDnumb,'LendingLibrary'=>$illiadLIBSymbol,'SpecIns'=>$specialinst,'TransactionStatus'=>'Awaiting Lending Request Processing','LoanTitle'=>$ititle,'LoanAuthor'=>$iauthor,'CallNumber'=>$itemcall,'LoanDate'=>$pubdate,'ILLNumber'=>$illnum ,'TAddress'=>$libreqname,'TAddress2'=>$libreqaddress2,'TCity'=>$libreqcity,'TState'=>$libreqcity,'TZip'=>$libreqzip,'TEMailAddress'=>$libreqemail);
                } else {
            ...
            if (mysqli_query($db, $sqlupdate2)) {
                ...
                $headers .= "MIME-Version: 1.0\r\n";
                $headers .= "Content-Type: text/html; charset=ISO-8859-1\r\n";
                $messagereq = "UPDATE `$cdlcSTAT` SET IlliadStatus = ".$illstatus.", IlliadTransID = ".$illiadtxnub." WHERE index = ".$sqlidnumb." ";
                $headers = preg_replace('/(?<!\r)\n/', "\r\n", $headers);
      ...
      Location: $itemlocation<br>
       $article<br><br>
     <a href='http://eform.cdlc.org/cancel?num=$illnum&a=3' >Do you need to cancel this request? </a>
     <br><br>
     ...
     $email<br>
      ...
      This is an automated message from the eForm. Responses to this email will be sent back to staff at Southeastern NY Library Resources Council. If you would like to contact the other library in this ILL transaction, email ".$destemail.".";
     ...
     $wphone<br>
    <br><br>
     Will you fill this request?  <a href='http://eform.cdlc.org/respond?num=$illnum&a=1' >Yes</a> &nbsp;&nbsp;&nbsp;&nbsp;<a href='http://eform.cdlc.org/respond?num=$illnum&a=0' >No</a><br>
      <br> <hr style='width:200px;text-align:left;margin-left:0'>
      <br>
     This is an automated message from the eForm. Responses to this email will be sent back to staff at Southeastern NY Library Resources Council. If you would like to contact the other library in this ILL transaction, email ".$email."
     <br>";
        ...
        echo "<br><br><a href='http://eform.cdlc.org'>Would you like to do another request?<a><br>";
    } else {
