...
$sqlselect = "SELECT *  FROM `$cdlcSTAT` WHERE `IlliadStatus` LIKE '%Shipped%' or `IlliadStatus` LIKE '%Shipped%'";
echo $sqlselect."\n";
...
er library in this ILL transaction";
    ...
    if (strpos($status, 'Item Shipped') !== false) {
        ...
        $sqlupdate2 = "UPDATE `$cdlcSTAT` SET  `DueDate` = '$dueDate' WHERE `index` = $sqlidnumb\n";
        echo $sqlupdate2;
...
//Get data about requests from database
$sqlselect = "SELECT *  FROM `$cdlcSTAT` WHERE `Timestamp` < CURRENT_TIMESTAMP - INTERVAL 7 MONTH and `IlliadStatus` LIKE '%Shipped%'";
$retval = mysqli_query($db, $sqlselect);
$GETLISTCOUNT = mysqli_num_rows($retval);
...
while ($row = mysqli_fetch_assoc($retval)) {
    ...
    $sqlupdate2 = "UPDATE `$cdlcSTAT` SET  `IlliadStatus` = 'Assume Compelete after 7 months' WHERE `index` = $sqlidnumb\n";
    echo $sqlupdate2;
    //do database update and see if there was an error
    if (mysqli_query($db, $sqlupdate2)) {
        echo "database was updataed";
