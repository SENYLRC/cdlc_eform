...
while ($row = mysqli_fetch_assoc($retval)) {
			...
			###Get the Destination 
            $GETLISTSQLDESTEMAIL="SELECT `ILLEmail` FROM `CDLC-EFORM-Library-Data` where loc LIKE '$destination'  limit 1";
				...
				if ( empty($issn)) $issn='';
					if ( empty($arttile)) $article='';
						...
						$email_to = implode(',', $destemailarray); 
                                                $headers = "From: eFrom <donotreply@cdlc.org>\r\n" ;
						...
						#####SEND a copy of EMAIL to requester with DKIM sig
                                                $headers = "From: Linx <donotreply@cdlc.org>\r\n" ;
                                                $headers .= "MIME-Version: 1.0\r\n";
                                                $headers .= "Content-Type: text/html; charset=ISO-8859-1\r\n";
